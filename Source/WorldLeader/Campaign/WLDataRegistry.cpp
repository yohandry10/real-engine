...
bool UWLDataRegistry::LoadGameData()
{
	...
	const bool bAmerica   = ValidateAmericaLowDetailCoverage(DataDir / TEXT("Campaign3D") / TEXT("AmericaLowDetail"));
	...
	return bProvinces && bNations && bBuildings && bUnits && bGoods && bValid && bAmerica;
}
...
bool UWLDataRegistry::ValidateAmericaLowDetailCoverage(const FString& DirectoryPath) const
{
	...
	if (FileNames.Num() == 0)
	{
		UE_LOG(LogWorldLeader, Error, TEXT("WLDataRegistry: AmericaLowDetail no contiene archivos en %s."), *DirectoryPath);
		return false;
	}
	...
	for (const FString& FileName : FileNames)
	{
		...
		if (!FFileHelper::LoadFileToString(Raw, *FilePath))
		{
			UE_LOG(LogWorldLeader, Error, TEXT("WLDataRegistry: no se pudo validar AmericaLowDetail %s."), *FilePath);
			bOk = false;
			continue;
		}
		...
		{
			UE_LOG(LogWorldLeader, Error, TEXT("WLDataRegistry: AmericaLowDetail invalido en %s."), *FilePath);
			bOk = false;
			continue;
		}
		...
		{
			const TSharedPtr<FJsonObject>* ObjPtr = nullptr;
			if (!CountryValue.IsValid() || !CountryValue->TryGetObject(ObjPtr) || !ObjPtr)
			{
				continue;
			}

			FString Iso;
			(*ObjPtr)->TryGetStringField(TEXT("iso"), Iso);
			...
			const FWLNationData* Nation = Nations.Find(Iso);
			if (!Nation)
			{
				UE_LOG(LogWorldLeader, Error, TEXT("WLDataRegistry: pais AmericaLowDetail %s no existe en Nations.json."), *Iso);
				bOk = false;
				continue;
			}

			bool bHasPlayableProvince = false;
			for (const TPair<FString, FWLProvinceData>& ProvincePair : Provinces)
			{
				if (ProvincePair.Value.CountryIso == Iso)
				{
					bHasPlayableProvince = true;
					break;
				}
			}
			if (!bHasPlayableProvince)
			{
				UE_LOG(LogWorldLeader, Error, TEXT("WLDataRegistry: pais AmericaLowDetail %s no tiene provincia en Provinces.json."), *Iso);
				bOk = false;
			}
		}
	}
